        ...
        <div class="col-md-4" v-for="testimonial in testimonials" :key="testimonial.id">
          <div class="card p-3 h-100">
            <div class="card-body">
              ...
              <h5 class="card-title">{{ testimonial.user }}</h5>
            </div>
            <div class="card-footer">
              <button v-if="user && user.email === testimonial.user" @click="editTestimonial(testimonial)" class="btn btn-secondary">Edit</button>
              <button v-if="user && user.email === testimonial.user" @click="deleteTestimonial(testimonial._id)" class="btn btn-danger">Delete</button>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  setup(props) {
    ...
    const postTestimonial = async () => {
      const testimonial = {
        user: props.user ? props.user.email : 'Anonymous',
    ...
    return { featuredCourses, specialCourses, testimonials, newTestimonial, postTestimonial, editMode, editableTestimonial, editTestimonial, cancelEdit, updateTestimonial, deleteTestimonial };
  },
};
</script>
